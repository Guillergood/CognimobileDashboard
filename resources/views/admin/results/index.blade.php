                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <div>
                                ...
                                <div class="dropdown">
                                    ...
                                    <button type="button" class="btn btn-info d-none" aria-expanded="false" onclick="saveScore({{ $result->_id}},{{$result->data}})">
                                        Save Edited Score
                                    </button>
                        ...
                        <div class="col-sm-12 col-md mb-sm-2 mb-0 card-footer">
                          <div class="text-muted">Score:</div><strong>{{ str_replace("}","",substr($result->data , strpos($result->data, "overall_score")+16, 2)) ?? '' }} (({{str_replace("}","",substr($result->data , strpos($result->data, "overall_score")+16, 2))*100)/36}}%)</strong>
                          <div class="progress progress-xs mt-2">
                            <div class="progress-bar bg-gradient-success" role="progressbar" style="width: {{(str_replace("}","",substr($result->data , strpos($result->data, "overall_score")+16, 2))*100)/36}}%" aria-valuenow="{{str_replace("}","",substr($result->data , strpos($result->data, "overall_score")+16, 2))}}" aria-valuemin="0" aria-valuemax="36"></div>
                          </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function onDropDown(buttonPressed, data) {
        ...
        let saveButton = parent.children[2];
        ...
        if(displayData.innerHTML.trim().length === 0){
    ...
    function createScoreSection(parent, score) {
        ...
        scoreValue.className="card-title mb-0 ml-1 scoreValue";
        ...
        saveButton.className = "btn btn-info ml-2 d-none"
    ...
    function createMemoryTab(testCardBody, memoryTaskInfo, taskTitle) {
        ...
        expectedAnswerText.innerText = "Answer: " + memoryTaskInfo.expected_answer.toString();
    ...
    function createLanguageCard(testCardBody, languageTaskInfo, taskTitle) {
        ...
        for(let i = 0; i < languageTaskInfo.answer.length; ++i){
            ...
            answerText.innerText = "Answer: " + languageTaskInfo.answer[i];
            expectedAnswerText.innerText = "Expected Answer: " + languageTaskInfo.expected_answer[i];
    ...
    function saveScore(id,json) {

        let MEMORY = 4;

        let values = document.getElementsByClassName("scoreValue");
        for(let i = 0; i < values.length; ++i) {
            if(json["data"][0]["task_"+i]["taskType"] !== MEMORY)
                json["data"][0]["task_"+i]["score"] = parseInt(values[i].innerText);
        }
