  ...
  fun getData(device_id: String, table: String, start: Double, end: Double): Future<JsonArray> {
    ...
    sqlClient.getConnection { connectionResult ->
      if (connectionResult.succeeded()) {
        ...
        connection.query("SELECT * FROM tests WHERE device_id ='a5c953fa-02ca-4c02-b84a-04526f404be0' AND timestamp between '0' AND '1586274244786' ORDER BY timestamp ASC") { result ->
          if (result.failed()) {

            println("Failed to retrieve data: ${result.cause().printStackTrace()}")
            println("Failed to retrieve data: ${result.cause().message}")
            println("Failed to retrieve data: ${result.cause()}")
            println("Failed to retrieve data: ${result}")
            connection.close()
